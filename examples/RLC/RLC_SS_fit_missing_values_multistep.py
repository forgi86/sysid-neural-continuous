...
if __name__ == '__main__':
    ...
    # Add missing values
    N = len(t)
    idx_missing = np.sort(np.random.choice(N, int(N*0.2), replace=False))  # indexes of missing values
    x_noise[idx_missing, :] = 0.0  # just any value here, it will be excluded from the loss anyways
    missing = np.zeros(t.shape, dtype=np.bool)
    missing[idx_missing] = True
    ...
    missing_torch_fit = torch.from_numpy(missing_fit)

    # Create hidden state
    x_hidden_init = np.copy(x_fit)
    x_hidden_init[np.isnan(x_hidden_init)] = 0.0 # just set to 0 missing hidden
    ...
    def get_batch(batch_size, seq_len):
        ...
        batch_missing = torch.tensor(missing_fit[batch_idx])

        return batch_t, batch_x0_hidden, batch_u, batch_x, batch_x_hidden, batch_missing


    # Scale loss with respect to the initial one
    with torch.no_grad():
        batch_t, batch_x0_hidden, batch_u, batch_x, batch_x_hidden, batch_missing = get_batch(batch_size, seq_len)
        batch_x_sim = nn_solution(batch_x0_hidden, batch_u)
#        traced_nn_solution = torch.jit.trace(nn_solution, (batch_x0_hidden, batch_u))
        err_init = batch_x_sim*~batch_missing - batch_x*~batch_missing
    ...
    for itr in range(0, num_iter):
        ...
        # Compute fit loss
        err_fit = batch_x_sim*~batch_missing - batch_x*~batch_missing
