...
from torchid.lstmfitter import LSTMDeepSimulator
...
if __name__ == '__main__':
    ...
    # Setup neural model structure0
    nn_solution = LSTMDeepSimulator(n_input=1, n_output=2)
    ...
    for itr in range(0, num_iter):
        ...
        x_sim_torch_fit = nn_solution(u_torch_fit[None, :, :])
    ...
    if not os.path.exists("models"):
        os.makedirs("models")
    if add_noise:
        model_filename = f"model_LSTM_fullsim_noise.pkl"
        hidden_filename = f"hidden_LSTM_fullsim_noise.pkl"
    else:
        model_filename = f"model_LSTM_fullsim_nonoise.pkl"
        hidden_filename = f"hidden_LSTM_fullsim_nonoise.pkl"

    torch.save(nn_solution.state_dict(), os.path.join("models", model_filename))
    ...
    with torch.no_grad():
        x_sim_torch_val = nn_solution(u_torch_fit[None, :, :])
        x_sim_torch_val = x_sim_torch_val.squeeze(0)
    ...
    if add_noise:
        fig_name = f"RLC_LSTM_loss_fullsim_noise.pdf"
    else:
        fig_name = f"RLC_LSTM_loss_fullsim_nonoise.pdf"
