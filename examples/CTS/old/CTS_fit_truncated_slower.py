...
if __name__ == '__main__':
    ...
    for itr in range(0, num_iter):
        ...
        if itr % test_freq == 0:
            with torch.no_grad():
                ...
                print(f'Iter {itr} | Tradeoff Loss {loss:.6f}   Consistency Loss {alpha*loss_consistency:.6f}   Fit Loss {loss_fit:.6f}   Simulation Loss {loss_sim:.6f}')
    ...
    # initial state has to be estimated, according to the benchmark description
    ...
    ax[0].plot(time_exp, x_hidden_fit_np[:, 1], 'b', label='${x_{2}}^h$') # may be excluded from the plot
    ax[0].legend(loc='upper right')
    ax[0].grid(True)
    ax[0].set_ylabel("Voltage (V)")

    ax[1].plot(time_exp, x_hidden_fit_np[:, 0], 'b', label='${x_{1}}^h$')
    ax[1].plot(time_exp, x_sim_val[:, 0], 'r', label='${x_{1}}^{\mathrm{sim}}$')
    ax[1].set_ylabel("$x_1$ (-)")
    ax[1].grid()
